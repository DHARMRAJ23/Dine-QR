 ...
 *            └─ <HashRouter>
 *               ├─ /menu/:tableId  ─ Customer: browse menu (MainMenu)
...
import MainMenu from './components/customer/MainMenu';
...
const AppContent: React.FC = () => {
  ...
  /**
   * Controls the width/layout of the offline banner.
   * Admin pages use full-width; customer pages use the narrow centred layout.
   */
  ...
  useEffect(() => {
    ...
    const handleHashChange = () => setIsAdmin(window.location.hash.startsWith('#/admin'));
  ...
  return (
    <Router>
      {!isOnline && (
        <div className={`sticky top-0 z-50 bg-red-600 text-white text-[11px] text-center p-2.5 flex flex-col items-center justify-center gap-1 shadow-md border-b border-red-700 font-sans ${isAdmin ? 'w-full' : 'max-w-md mx-auto border-x border-red-700'}`}>
          <div className="flex items-center gap-1.5 font-bold uppercase tracking-wider">
            <WifiOff size={13} />
            <span>Connection lost</span>
          </div>
          <p className="text-[9px] text-red-100/90 leading-tight">
            Orders placed offline will be saved locally on this device only. Do not clear browser history or use private browsing mode, as this will result in order data loss.
          </p>
        </div>
      )}
      <Routes>
