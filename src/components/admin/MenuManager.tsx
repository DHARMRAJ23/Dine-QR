...
export const MenuManager: React.FC = () => {
  const { menuItems, toggleItemAvailability, addMenuItem, updateMenuItem, deleteMenuItem } = useCart();
  ...
  const handleOpenAddModal = () => {
    ...
    setImage('https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&q=80&w=400'); // Default appetizing pic
    setIsVeg(true);
    setFormError('');
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const isApprovedDataUrl = /^data:image\/(webp|jpeg|png);base64,/.test(trimmedImage);
    const isHttpsUrl = /^https:\/\//.test(trimmedImage);

    if (!isApprovedDataUrl && !isHttpsUrl) {
      setFormError('Image source must start with https:// or be a valid uploaded image.');
      return;
    }
    ...
    if (isApprovedDataUrl && trimmedImage.length > 700_000) {
      setFormError('Processed image is too large. Please choose a smaller image.');
      return;
    }
    ...
    <div className="flex h-screen bg-slate-950 overflow-hidden font-sans text-slate-100">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header className="p-6 border-b border-slate-800 bg-slate-900/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 flex-shrink-0">
          <div>
            <h1 className="font-display font-bold text-xl text-white">Menu Manager</h1>
            <p className="text-xs text-slate-500 mt-0.5 font-light">Add, edit, or toggle availability of items on the customer menu</p>
          </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto min-h-0 bg-slate-950">
          {filteredItems.length > 0 ? (
                ...
                >
                  <div>
                    ...
                    <div className="relative h-40 bg-slate-950 rounded-xl overflow-hidden mb-4 border border-slate-800">
                      ...
                      {/* Veg / Non-Veg Badge */}
                      <span className={`absolute top-3 left-3 px-2 py-0.5 rounded text-[9px] font-extrabold uppercase tracking-wider text-white border ${
                        item.isVeg 
                    ...
                    <div className="flex justify-between items-start gap-2">
                      <h3 className="font-semibold text-white text-[15px] line-clamp-1">{item.name}</h3>
                      <span className="font-extrabold text-orange-500 text-[14px]">₹{item.price}</span>
                    </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 text-slate-500 text-center">
              ...
              <h3 className="font-bold text-slate-400 uppercase tracking-wider text-xs">No Items Found</h3>
          ...
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h2 className="text-base font-bold text-white tracking-wide flex items-center gap-1.5">
                <Sparkles size={16} className="text-orange-500 fill-orange-500/20" />
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Item Name</label>
                  <input
                    type="text"
                    value={name}
                ...
                {/* Price */}
                <div className="space-y-1.5">
                  <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Price (₹)</label>
                  <input
                    type="number"
                    value={price}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Category Selector */}
                <div className="space-y-1.5">
                  <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Category</label>
                  <select
                    value={category}
                ...
                <div className="space-y-1.5">
                  <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Food Preference</label>
              ...
              <div className="space-y-2">
                <label className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Food Image Source</label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    <span className="text-[10px] text-slate-500 block">Upload Local File</span>
                  ...
                  {/* URL Text input */}
                  <div className="space-y-1">
                    <span className="text-[10px] text-slate-500 block">Or Paste Image URL</span>
                    <input
                      type="text"
                      value={image}
