...
if (!supabaseUrl || !supabaseKey) {
  console.error("Error: VITE_SUPABASE_URL and either SUPABASE_SERVICE_ROLE_KEY or VITE_SUPABASE_ANON_KEY must be set.");
  console.error("Run this script using Node.js 20.6+ with env loading, e.g.:");
  console.error("  node --env-file=.env.local seed-db.js");
  process.exit(1);
}

if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
  console.warn("Warning: Running without SUPABASE_SERVICE_ROLE_KEY. DB insertions may fail due to RLS policies.");
}
...
async function seed() {
  ...
  const { data: table, error: tErr } = await supabase
    ...
    .insert([{ restaurant_id: restaurant.id, table_number: 1, qr_token: 'table-1-qr-xyz' }])
    .select()
    .single();
  ...
  const { error: mErr } = await supabase.from('menu_items').insert(menuItemsToInsert);
