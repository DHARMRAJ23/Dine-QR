...
export const CartPage: React.FC = () => {
  ...
  // Tax calculations
  const gstTax = Number((itemTotal * BILLING_CONFIG.taxRate).toFixed(2));
  const grandTotal =
    itemTotal > 0
      ? Number((itemTotal + gstTax + BILLING_CONFIG.serviceCharge).toFixed(2))
      : 0;
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      setCheckoutError(
        err.message || "An error occurred while placing your order.",
      );
  ...
  const handleBackToMenu = () => {
    navigate(`/menu/${tableToken || tableId || "1"}`);
  };

  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-32 relative w-full flex flex-col">
      {/* Header Bar */}
      ...
      {cart.length > 0 ? (
          ...
          <div className="md:grid md:grid-cols-3 md:gap-8">
            <div className="md:col-span-2 space-y-4">
              ...
              <div className="bg-orange-50 border border-orange-100 rounded-xl p-3 flex justify-between items-center text-xs">
                <span className="font-semibold text-orange-800 uppercase tracking-wider">
                  Ordering from
                </span>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        {!menuItems.find((m) => m.id === ci.item.id)
                          ?.isAvailable && (
                      ...
                      <div className="flex flex-col items-end justify-between h-full gap-2">
                        ...
                        <div className="flex items-center text-xs border border-slate-200 rounded-lg overflow-hidden bg-white shadow-sm font-bold text-orange-600">
                          ...
                          <span className="px-2 text-slate-800 text-[11px] tabular-nums bg-slate-50/50">
                            {ci.quantity}
                          </span>
                          <button
                            onClick={() =>
            ...
            <div className="md:col-span-1 space-y-4 mt-4 md:mt-0">
              ...
              <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm space-y-2.5">
                ...
                {nameError && (
                  <p className="text-[10px] text-red-500 font-semibold mt-1">
                    Your name is required so the waitstaff can identify you at
                    Table {tableId}!
                  </p>
                )}
              </div>
              ...
              <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm space-y-2 text-xs">
                <h3 className="font-bold text-slate-800 uppercase tracking-wider mb-2.5">
                  Bill Breakdown
                </h3>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
          ...
          <p className="text-xs text-slate-400 mt-1.5 max-w-[260px] leading-relaxed">
            Looks like you haven't added anything to your cart yet. Scan the
            menu and add items to your liking.
          </p>
          <button
            onClick={handleBackToMenu}
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-0 right-0 md:right-8 z-40 w-full p-4 max-w-[calc(100vw)] md:max-w-sm ml-auto bg-gradient-to-t from-slate-50 via-slate-50 to-transparent">
          <button
