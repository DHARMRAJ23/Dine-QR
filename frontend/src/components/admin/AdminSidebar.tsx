...
export const AdminSidebar: React.FC = () => {
  ...
  const { resetMenuToDefaults, clearAllOrders, resetCompleteDemo, clearCart } =
    useCart();
  ...
  const handleResetMenu = () => {
    if (
      window.confirm(
        "Reset menu items to default list? Your custom additions/changes will be lost.",
      )
    ) {
      resetMenuToDefaults();
    }
  ...
  const handleClearOrders = () => {
    if (
      window.confirm(
        "Are you sure you want to clear all live orders? This will empty the Order Board.",
      )
    ) {
      clearAllOrders();
    }
  ...
  const handleResetDemo = () => {
    if (
      window.confirm(
        "CAUTION: This will clear all orders, reset the menu to default, empty the cart, and sign you out. Proceed?",
      )
    ) {
      resetCompleteDemo();
    }
    ...
    <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col h-screen border-r border-slate-800 flex-shrink-0 no-print">
      ...
      <div className="p-6 border-b border-slate-800 flex items-center justify-between">
        <div>
          ...
          <span className="text-[10px] uppercase font-bold text-slate-500 tracking-widest mt-0.5 block">
            Kitchen Console
          </span>
        </div>
      </div>
      ...
      <div className="px-4 py-3 border-t border-slate-800/60 bg-slate-950/20 text-slate-400 flex flex-col gap-2">
        <span className="text-[9px] uppercase font-bold tracking-widest text-slate-500 block px-1.5">
          Demo Controls
        </span>
